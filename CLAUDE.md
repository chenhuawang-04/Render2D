...
- `third_party/freetype/`, `third_party/harfbuzz/`, and `third_party/sheenbidi/` are **git submodules** (pinned to `VER-2-14-3` / `14.2.1` / `v3.0.0`) for the **Stage 19** font/text runtime — run `git submodule update --init --recursive` after cloning. They build as static libraries behind the `RENDER2D_BUILD_FONT_RUNTIME` option (default ON) and are exposed only through the internal `render2d_font_runtime_support` target — the same isolation as `render2d_thread_runtime_support`. **Never include FreeType/HarfBuzz/SheenBidi headers from the public umbrella `Render2D.hpp`**; font-runtime headers live under `include/Render2D/Font/` and require linking `render2d_font_runtime_support`. The Stage 9 glyph systems and the Stage 19C pure systems (`include/Render2D/System/TextShapingSystem.hpp`) stay deterministic and dependency-free.
- `third_party/sdl/` is a **git submodule** (SDL3, pinned to `release-3.4.10`) for the **Stage 22** present-host (on-screen presentation). It builds as a static library (video + Vulkan only) behind the `RENDER2D_BUILD_PRESENT_HOST` option (default ON) and is exposed only through the internal `render2d_present_host_support` target — the same isolation as the font/thread runtimes. **Never include SDL headers from the public umbrella `Render2D.hpp`**; present-host headers live under `include/Render2D/Present/` and require linking `render2d_present_host_support`. Render2D's core stays windowless — SDL only provides a host window + `VkSurfaceKHR` to feed the (unchanged) swapchain/present runtimes, and a host engine sets `RENDER2D_BUILD_PRESENT_HOST=OFF` to supply its own surface at merge. Do not edit anything under `third_party/sdl/` (SDL's own `CLAUDE.md` forbids AI-authored contributions; we only consume it).
- `third_party/stb/` is a **vendored single-header pair** (`stb_image.h` / `stb_image_write.h`, public domain — like `third_party/renderdoc`, *not* a submodule; the committed copy is the pin) used **only by test scaffolding**. It is compiled in exactly one TU behind the internal `render2d_test_image_io` target as a **SYSTEM** include and **must never be included from the umbrella `Render2D.hpp` or anywhere in `include/Render2D`/`src`**. It backs the **test-only lightweight ECS + asset framework** (`tests/support/MiniEcs.hpp`, `tests/support/AssetRegistry.hpp`, `tests/support/ImageFile.{hpp,cpp}`) and its tests (`render2d.mini_ecs_pipeline`, `render2d.asset_scene_render`). These prove the renderer is usable end-to-end from an author-it-yourself scene, but a production ECS and asset pipeline remain **out of scope for the core** (the host engine's job at merge) — this is test-only scaffolding, exactly like `HostLikeEcs`. See `docs/adr/2026-06-19-test-ecs-asset-framework.md`.
