...
- `include/Render2D/Present/PresentHost.hpp` - Stage 22A SDL3 present-host build/link foundation: queries the compiled vs. linked SDL version (`presentHostCompiledSdlVersion`/`presentHostLinkedSdlVersion`) to prove the SDL submodule builds, the internal target links it, and the headers are reachable — no window/display/GPU touched. The only SDL `#include` in the tree; links `render2d_present_host_support`, never in the umbrella. The real window/surface/swapchain/present API arrives in 22B.
