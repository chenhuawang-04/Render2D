...
- `tests/support/PresentBringup.hpp` - Test-only present-host bring-up helpers shared by the Stage 22B/22C present tests: turn an SDL-provided `VkSurfaceKHR` into a present-capable `VkInstance`/`VkDevice` (`createPresentInstance`, `selectPresentDevice`, `createPresentDevice`) and pick a swapchain surface format / composite alpha (`selectSurfaceFormat`, `selectCompositeAlpha`, `clampU32`). Not production architecture — the host engine owns instance/device creation at merge.
